...
import {any, array, bool, enumerated, int, sfloat, string, tuple,} from './types';

const weapons = enumerated(['no_weapon', 'axe']);
const shields = enumerated(['no_shield', 'shield_wooden']);
const citizen_types = enumerated(
    Object.keys(constants.minions) as any as
    (keyof(typeof constants)['minions'])[]);
...
export const constructors_object = Object.fromEntries(constructors.map(
                                       ([constructor, props]) =>
                                           [constructor,
                                            Object.fromEntries(props),
])) as ConstructorsToObject<typeof constructors>;

export const constructors_keys = constructors.map(([name]) => name) as
    ExtractConstructorNames<typeof constructors>;

export const constructors_inner_keys =
    Object.fromEntries(constructors.map(
        ([name, props]) =>
            [name,
...
export type ConstructorsObject = {
  [K in keyof typeof constructors_object]: {
    [P in keyof(typeof constructors_object)[K]]:
        (typeof constructors_object)[K][P] extends {
...
type ConstructorsToObject<
    T extends readonly(
        readonly[string, readonly(readonly[string, any])[]])[]> = {
  [K in T[number][0]]:
...
type ExtractNetworkedType<T> =
    T extends readonly[infer Conv extends(...args: any) => any, any] ?
    Parameters<Conv>[0] :
    T extends(...args: any) => infer R ? ExtractNetworkedType<R>: never;

type MapConstructor<T> = T extends readonly[infer Name, infer Props] ?
    [
      Name, Props extends readonly(readonly[infer _, infer _])[] ?
          {
...
type ExtractConstructorNames<T> =
