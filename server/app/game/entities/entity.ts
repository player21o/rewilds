import {constructors_inner_keys, constructors_keys, constructors_object, ConstructorsInnerKeys, ConstructorsObject,} from '../../../../common/constructors';
...
> extends GameObject {
  ...
  public update(dt: number, c: Collisions) {
    ...
    const updates = !this.new_one ? this.constructor_properties.map((prop) => {
      const propName = prop as keyof typeof constructor;
      const converterPair = constructor[propName] as
          readonly[(val: any) => any,
                   (val: any) => any,
  ...
  /**
   * a function to set some value inside entity *outside* update loop (e. g. as
   * an answer to packet)
   */
  ...
  public set<T extends keyof typeof this>(
      ...
      value:|
      ((prev_value:
            ToPrimitive<(typeof this)[T]>) => ToPrimitive<(typeof this)[T]>)|
      ToPrimitive<(typeof this)[T]>) {
    if (typeof value == 'function') {
      this.to_set[key as any] =
          value(key in this.to_set ? this.to_set[key as any] : this[key]);
    } else {
