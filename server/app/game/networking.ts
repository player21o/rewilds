...
export class GameNetworking {
  ...
  constructor(
      port: number, onOpen: (peer: Peer) => void,
    ...
    this.app =
        App({})
            .ws('/*', {
              ...
              message: (ws: Ws, msg) => {
                const packet: [packet: number, any[]] =
                    unpack(new Uint8Array(msg)) as any;

                const formatted:
                    ConstructorsInnerTypes[keyof ConstructorsObject][] = [];
