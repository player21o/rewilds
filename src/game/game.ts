  ...
  private ticker_cb(assets: ObjectManifest['bundles']['game']) {
    return (ticker: Ticker) => {
      ...
      this.debug_sprites[0].position.set(
          this.deps.viewport.left, this.deps.viewport.top);
      ...
      this.deps.entities.forEach((e) => {
        if (e.x < this.deps.viewport.left ||
      ...
      if (this.deps.me.citizen != null) {
        ...
        this.lastX = lerp(
            this.lastX, this.deps.me.citizen.x + this.app.renderer.width / 2,
            t);

        this.lastY = lerp(
            this.lastY, this.deps.me.citizen.y + this.app.renderer.height / 2,
            t);
